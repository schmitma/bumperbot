    ...
    : Node(name), x_increment_(0.05), x_last_(0.0), rotations_counter_(0)
{
  ...
  RCLCPP_INFO_STREAM(
      get_logger(), "Publishing static transform between " << static_transform_stamped_.header.frame_id << " and "
    ...
    const std::shared_ptr<bumperbot_msgs::srv::GetTransform::Response> res)
{
  ...
  {
    RCLCPP_ERROR_STREAM(
        get_logger(), "An error occurred while transforming from " << req->frame_id << " and " << req->child_frame_id
                                                                   << ": " << ex.what());
    res->success = false;
    return true;
  }
